...
function addWeekdayRow() {
  ...
  row.querySelector('.remove-btn').addEventListener('click', () => { row.remove(); updatePreview(); }); weekdayList.appendChild(row); updatePreview();
}
...
async function uploadToFtp() {
  ftpStatusEl.textContent = 'Uploading to FTP server...';
  try {
    const response = await fetch('/api/upload', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ directoryName: sanitizeDirectoryName(buildPayload().title), event: buildPayload(), files: await filesAsBase64() }) });
    const result = await response.json();
    if (!response.ok || !result.ok) throw new Error(result.error || 'FTP upload failed.');
    ftpStatusEl.textContent = `Uploaded successfully to ${result.remoteDirectory}`;
...
document.getElementById('downloadPackageBtn').addEventListener('click', async () => { const payload = buildPayload(); const directoryName = sanitizeDirectoryName(payload.title); const zip = new JSZip(); const folder = zip.folder(directoryName); folder.file('event.json', JSON.stringify(payload, null, 2)); selectedFiles.forEach(file => folder.file(sanitizeFileName(file.name), file)); downloadBlob(await zip.generateAsync({ type: 'blob' }), `${directoryName}.zip`); });
document.getElementById('ftpUploadBtn').addEventListener('click', uploadToFtp);
