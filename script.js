...
document.getElementById('addWeekdayBtn').addEventListener('click', addWeekdayRow); document.getElementById('previewBtn').addEventListener('click', updatePreview); eventForm.addEventListener('input', updatePreview); eventForm.addEventListener('change', updatePreview); document.getElementById('downloadJsonBtn').addEventListener('click', () => { const payload = buildPayload(); downloadBlob(new Blob([JSON.stringify(payload, null, 2)], { type: 'application/json' }), jsonFileName(FIXED_LOCATION_ID)); }); document.getElementById('downloadPackageBtn').addEventListener('click', async () => { const payload = buildPayload(); const directoryName = sanitizeDirectoryName(payload.title); const zip = new JSZip(); const folder = zip.folder(directoryName); folder.file(jsonFileName(FIXED_LOCATION_ID), JSON.stringify(payload, null, 2)); selectedFiles.forEach(file => folder.file(sanitizeFileName(file.name), file)); downloadBlob(await zip.generateAsync({ type: 'blob' }), `${directoryName}.zip`); }); document.getElementById('ftpUploadBtn').addEventListener('click', uploadToFtp); document.getElementById('resetBtn').addEventListener('click', hardReset);
